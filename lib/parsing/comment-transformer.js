...
class CommentTransformer {
  ...
  /**
   * Inserts a given comment string into the transformer, to prep for
   * being transformed.
   *
   * @param {string} comment The comment text to load into the transformer.
   * @throws {TypeError} If comment is not a string
   */
  load(comment) {
    if (!(typeof comment === "string") && !(comment instanceof String)) {
      throw new TypeError("CommentTransformer.load() requires a string.");
    }
  ...
  transform(comment) {
    ...
    // TODO: Extract property names and value
