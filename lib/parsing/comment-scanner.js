  ...
  scan() {
    ...
    while (this.#notEOS()) {
      ...
      if (currentChar === "<" && !inString) {
        // If comment seems to be coming up, from comment open symbol,
        // scan and push relevant segments.
        if (this.#commentOpenNext()) {
          const comment = this.#scanComment();

          // If not actually a proper comment, ignore and treat as normal text.
          if (comment != null) {
